...
void printCSV(const string& filename) {
    ...
    // Determine indices of requested columns
...
int main( int argc, char *argv[ ] ) {
    ...
    cout << "Commande execute :\t   " << cmd_param << "\n";
    cout << "Output file destination :  " << dataFile << "\n";

    // Lancer mojitos en arrière-plan et récupérer son PID
    string cmd_mojitos = "sudo ../mojitos/bin/mojitos " 
                        + string(PARAM_MOJITOS) 
                        + " -o " 
                        + dataFile 
                        + " & echo $!";
    ...
    // Exécuter la commande utilisateur
    int ret = system(cmd_param);
    cout << "User command completed" << endl;

    // Tuer le processus mojitos
